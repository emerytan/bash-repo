...
tput setaf 11
echo -e "\tdirectory count prior to rename:\t\t\t$FileCount "
if [ "$COUNT" != "0" ]; then
...
if [ "$CNF" != "y" ]; then
    ...
    ls -1 | head | grep $matchTo | while read FILEDIR; do
        #DIR="${FILEDIR}"
        FILE="${FILEDIR/*\//}"
...
tput setaf 11
echo -e "\tfile count pre rename:\t\t$FileCount "
echo -e "\tfile count post rename:\t\t$PostFileCount "
if [ $COUNT == 0 ]; then
