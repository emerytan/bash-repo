...
sourcePath="/V01/panama/CameraRaw/FROM_CO3/SCANS"
...
cronLog="${hashPath}/cronLog/tailLog.txt"

cd $sourcePath
currentDir=$(pwd)

if [ $sourcePath == $currentDir ]; then
	ls ${hashFile} | tail -n 8 >> ${cronLog}
	START=$(date)
	echo "Starting... ${START}" >> ${cronLog}
    	relPath="md5Check/noTab_MD5"
	for tape in $(ls ${hashFile} | tail -n 8); do
...
chown -R root:cameraraw ${hashPath}
